...
UserHandling.prototype = {
	...
	onUserLogin: function(socket, data) {
		...
		if(data.username){
			...
			socket.emit('userlogin'); that.onUserList(socket);
		}else{
	...
	onUserList: function(socket, data) {
		...
		keys.forEach(function(k) {
			userList[i++] = { id: k, username: that.loggedUsers[k].handshake.username };
		});
	...
	onMakePost: function(socket, data) {
		var id = Date.now(),
		postData = { author: socket.handshake.username, authorId: socket.id, timestamp: id, id: id, text: data };
		this.posts[id+''] = 0;
	...
	onMakeComment: function(socket, data) {
		var id = Date.now(),
		commentData = { author: socket.handshake.username, authorId: socket.id, timestamp: Date.now(), text: data.text, postId: data.postId, id: id };
		this.posts[id+''] = 0;
